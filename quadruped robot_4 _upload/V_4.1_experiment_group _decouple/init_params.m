function p = init_params()
    ...
    p.kf_R = 0.03^2*19.974;            % 测量噪声基准 (对应传感器 3cm)

    % 8.========== 【新增】待优化的次级超参数 (请修改这里) ==========
    % 把"解耦优化"或"手动选取"得出的最优参数填在这里覆盖默认值
    % [感知层 KF]
    ...
    % [规划层 Margin] (做 Step 1 验证时，这部分可以用默认值)
    p.sm_decay          = 1.5936e-6;   
    p.sm_penalty        = 0.07905;  
    p.sm_max            = 0.12256;
end