function p = init_params()
    ...
    p.kf_R = 0.03^2*96.286;            % 测量噪声基准 (对应传感器 3cm)

    % ========== 【新增】待优化的次级超参数 (默认值) ==========
    % [感知层 KF]
    p.kf_thresh_noise   = 0.36506;   % 判定为传感器故障的阈值 (原 0.30)
    p.kf_scale_noise    = 473.41;  % 故障时 R 放大的倍数 (原 100)
    p.kf_thresh_step    = 0.041281;   % 判定为台阶的阈值 (原 0.04)
    p.kf_scale_step     = 0.83649;    % 台阶时 R 缩小的倍数 (原 0.1)
    
    % [规划层 Margin]
    p.sm_decay          = 1.0935e-6;   % 遗忘速率 (原 0.00005)
    p.sm_penalty        = 0.061123;  % 痛觉敏感度 (原 0.005)
    p.sm_max            = 0.17714;   % 最大余量限制 (原 0.08)
end